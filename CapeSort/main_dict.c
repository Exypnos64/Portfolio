...
#define SORT_TYPE int
#define SORT_CMP(x, y)  (x - y)
void cape_sort_dict(SORT_TYPE* dst, int size, FILE* dictionary, char* capeSeq, char* outSamples, char* outSort, int repeated, int seqSize) {
/*void cape_sort_dict(SORT_TYPE* dst, int size, FILE* smallDict, FILE* largeDict, char* outSamples, char* outSort, int repeated, int seqSize) { */
	/*---------* Start Cape Sort *---------*/
/*	FILE* dictionary;*/
	printf("Cape Log:\n");
...
/*	}*/
	...
	int groups = seqSize; // The number of windows we look at: samples / window (hardcoded)
...
float test_cape(int * z_array, int * r_array, int size, FILE* dictionary, char* capeSeq, char* outSamples, char* outSort, int repeated, int seqSize) {
/*float test_cape(int * z_array, int * r_array, int size, FILE* smallDict, FILE* largeDict, char* outSamples, char* outSort, int repeated, int seqSize) {*/
	double start, end, total = 0;

	memcpy(z_array, r_array, size * sizeof(int));
	start = utime();
/*	cape_sort_test(z_array, size, quadCount, timCount, tieCount);*/
	cape_sort_dict(z_array, size, dictionary, capeSeq, outSamples, outSort, repeated, seqSize);
/*	cape_sort_dict(z_array, size, smallDict, largeDict, outSamples, outSort, repeated, seqSize);*/
	end = utime();
	total += end - start;
	verify(z_array, size);
...
/*				float time = test_cape(z_array, r_array, max, &quadCount, &timCount, &tieCount);*/
				float time = test_cape(z_array, r_array, max, dictionary,
						capeSeq, outSamples, outSort, repeated, parts);
